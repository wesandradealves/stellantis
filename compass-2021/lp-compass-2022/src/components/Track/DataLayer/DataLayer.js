...
export class DataLayer extends PureComponent {
  ...
  static push(idOrInfo, ...params) {
    if (!DataLayer.tracks) {
      return console.warn(
        'You should to call "DataLayer.init" method in order to send a push to %s.',
        idOrInfo,
      );
    }
    if (
      isString(idOrInfo) &&
      isFunction(DataLayer.tracks[idOrInfo])
    ) {
      const fnReturn = DataLayer.tracks[idOrInfo](
        ...params,
        ...DataLayer.args,
      );
  ...
  static applyPush(idOrInfo, args) {
    DataLayer.push.apply(
      DataLayer.push,
      [idOrInfo].concat(args),
    );
  }
  ...
  renderChildren() {
    if (this.props.log)
      console.log('DataLayer::', this.props.log);
